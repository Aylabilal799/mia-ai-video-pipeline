...
class SubtitleStyle(BaseModel):
    ...
    highlight_color: str = "#39FF14"  # karaoke 逐词高亮颜色（当前朗读词），默认发光绿

    # Max caption text width, as a fraction of video_width. Used by BOTH the
    # karaoke renderer and the plain-caption fallback so long captions wrap
    # consistently and never extend past the visible canvas on either path.
    # 0.83 of a 1080px-wide canvas ≈ 895px, matching the ~850-900px "Shorts
    # safe width" convention.
    max_width_ratio: float = 0.83
...
class SubtitleConfig(BaseModel):
    ...
    # v2.0 字幕对齐：启用 TTS 词级 cues 精确时间线（False=回退 legacy 启发式）
...
class ManuscriptVideoTask(BaseTaskState):
    ...
    manuscript_text: str = ""
    # 固定角色锚点图（i2i identity anchor）。若设置，每个场景视频生成都会
    # 携带该参考图，使生成的人物在所有分段/所有视频之间保持同一身份。
    reference_image: str = ""
    # 固定角色外观描述（如"Mia, 26岁健身博主..."）。会作为 <style> 注入每个
    # 场景的视觉 prompt 生成，与 reference_image 共同锁定角色身份/外观。
    character_style: str = ""
    # 固定随机种子（可选）。与 reference_image 一起使用能进一步降低生成结果
    # 的随机漂移，帮助角色在多个视频之间外观更稳定。
    ...
    combined_subtitle_words: str = ""   # 逐词时间戳 JSON 路径（karaoke 高亮字幕用）
    subtitle_styles_path: str = ""      # LLM 样式 JSON 路径（Phase 2）
