...
class BasePipeline(ABC):
    ...
    async def _recover_sub_maker(
        ...
        try:
            ...
            # Kokoro has no separate "harvest cues without audio bytes" mode
            # (unlike edge_tts's streaming protocol) -- generating into a
            # throwaway temp file is the only way to recover cues here.
            import tempfile
            with tempfile.NamedTemporaryFile(suffix=".mp3", delete=False) as tmp:
                tmp_path = tmp.name
            try:
                _, sub_maker = await KokoroTTSEngine().generate(
                    text=narration_text,
                    output_path=tmp_path,
                    voice=getattr(audio_config, "voice", "af_heart"),
            ...
            logger.warning("[Subtitle] recover sub_maker via Kokoro regeneration failed: %s", e)
            return None
    ...
    async def _generate_audio_with_fallback(
        ...
        """统一 TTS 音频生成（Kokoro，唯一引擎；失败即抛出，不再静默降级）。

        S2 收敛目标：替代 multi_scene / creative / poetry / manuscript / anchor
        五处复制的 TTS 生成逻辑（v5.0 Batch 2）。Edge-TTS 已被完全移除。

        行为矩阵：
            1. text 为空：empty_placeholder="" → 跳过返回 None（不落盘）；
               非空 → 以占位文本直接走 Silent 落盘（这不是 TTS，是"没有旁白
               文本"这一种独立场景，与音频引擎选择无关）。
            2. audio_config.enabled → KokoroTTSEngine.generate：
               成功 → 校验 cues（无逐词时间戳 → warning 并返回 None，
               字幕回退 legacy 启发式）；
               失败 → 直接向上抛出 RuntimeError，任务清楚地失败并报告真实
               错误（按用户明确要求："If Kokoro fails, the task should fail
               clearly and report the actual error"，不再像旧代码那样悄悄
               换成无声视频）。
            3. 音频关 + 字幕开 + harvest_cues_when_audio_off（路径 B）：
               通过 Kokoro 重新生成一次取 cues（RuntimeError 仅警告，因为这
               里音频本身是关闭的，只是想要字幕时间轴）→ Silent 落盘，返回 cues。
        ...
        from core.audio.tts import KokoroTTSEngine, SilentTTSEngine
        ...
        # 空文本：无占位 → 跳过；有占位 → 直接 Silent 落盘（这与 TTS 引擎无关，
        # 只是"这个场景没有配音文本"的独立场景）
        if not text:
        ...
        # 路径 A：音频开 → Kokoro 生成。失败即向上抛出，不再静默降级为无声视频。
        if audio_enabled:
            kokoro = KokoroTTSEngine()
            _, sub_maker = await kokoro.generate(
        ...
        # Kokoro 没有"只要 cues 不要音频字节"的流式接口，所以生成到临时文件
        # 后立刻丢弃；这里的 RuntimeError 仅警告（音频本来就是关闭的，只是
        # 想拿字幕时间轴，拿不到就回退 legacy 字幕，不影响主流程）。
        if subtitle_enabled and harvest_cues:
            ...
            try:
                _, sub_maker = await KokoroTTSEngine().generate(
                    text=text, output_path=tmp_path, voice=voice, rate=rate,
                )
            except RuntimeError as e:
                logger.warning("[Audio] Kokoro harvest_cues failed, silent fallback: %s", e)
            finally:
                if os.path.exists(tmp_path):
                    try:
                        os.remove(tmp_path)
