...
class ConcatMixin:
    ...
    def _parse_srt_to_clips(
        ...
        logger.info(f"[CAPTIONS] Font size: {subtitle_style.fontsize}")
        ...
        # Max caption width -- same max_width_ratio knob the karaoke
        # renderer uses (models/task.py::SubtitleStyle.max_width_ratio),
        # so both caption rendering paths respect the same configured
        # safe width instead of the karaoke path alone being fixed while
        # this fallback path used a different hardcoded margin.
        max_width_ratio = getattr(subtitle_style, "max_width_ratio", None)
        if max_width_ratio:
            available_w = max(int(video_width * max_width_ratio), 100)
        else:
            available_w = video_width - 40
        logger.info(f"[CAPTIONS] Max width: {available_w}px (video_width={video_width})")
        ...
        with open(srt_path, "r", encoding="utf-8") as f:
            for sub in srt_lib.parse(f):
                ...
                # 长文本自动拆为多行，避免单行溢出屏幕。传入真实
                # video_width/fontsize，让 _split_long_text 用宽度感知预算
                # (实际像素测量的近似) 而不是粗略的“1 字符≈1 fontsize px”
                # 猜测 -- 之前未传这两个参数，导致该函数退回到不区分字体/
                # 字重的粗略字符数估算，对 ArchivoBlack 这种较宽的粗体字
                # 会低估实际像素宽度。
                wrapped = SubtitleGenerator._split_long_text(
                    txt, max_chars_per_line=max(8, available_w // fs) if fs > 0 else 14,
                    video_width=available_w + 40, fontsize=fs,
                )

                # method="caption" + size=(available_w, None) makes moviepy
                # ALSO re-wrap using true Pillow font-metric measurements
                # (see TextClip.__break_text), so even if the pre-wrap above
                # is imperfect, the final rendered text is still guaranteed
                # to fit within available_w and never clip off the canvas.
                clip = MpTextClip(
                    text=wrapped,
                    font=font_path,
                    font_size=fs,
