...
# Bare "bottom"/"top" (no explicit "-N"/"+N" offset) resolve to this many
# pixels of margin from the respective frame edge, NOT flush against it.
# Matches the SubtitleStyle model's own default ("bottom-80").
DEFAULT_SUBTITLE_BOTTOM_MARGIN_PX = 80
DEFAULT_SUBTITLE_TOP_MARGIN_PX = 80

# Default max caption width as a fraction of video_width, mirroring
# SubtitleStyle.max_width_ratio's default. Kept here too so non-model
# callers (e.g. quick scripts) have a single source of truth.
DEFAULT_SUBTITLE_MAX_WIDTH_RATIO = 0.83
...
# D3：可选语音角色列表（Kokoro 静态目录，见 core.audio.voices）。
