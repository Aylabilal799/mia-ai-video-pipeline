...
# LINE-grouping bounds for karaoke captions (see _group_items_for_karaoke /
# generate_karaoke_word_data below). Tuned for short ~1-4 word CapCut/TikTok
# style caption bursts (each burst replaced by the next as speech advances),
# rather than a longer multi-word line with word-by-word highlight moving
# across it.
_MAX_SUB_DURATION = 1.1
# 每条字幕最大字符数（中文场景）— sized for a ~1-4 word caption burst
# (avg ~5.5 chars/word incl. space -> 4 words ~= 22 chars; capped a bit
# under that so it skews toward 2-3 words rather than always hitting 4).
_MAX_SUB_CHARS = 18
# 最少字数字幕阈值：如果词级 cues 太少（如只有 3 个 cues for 14s），
# 说明 TTS 引擎本身提供的粒度已足够，不需要额外细化（避免空洞字幕）
...
class SubtitleSrtMixin:
    ...
    def _generate_fine_srt_from_word_cues(
        ...
        将 TTS 词级 cues（时间戳列表）分组为短字幕段落，
        每组不超过 max_duration 秒和 max_chars 字符，优先在较长停顿处断开。

        Args:
            word_cues: TTS 词级 cues 列表（.start/.end/.content 结构）
    ...
    @staticmethod
    def _group_items_for_karaoke(
        items: List[Tuple[float, float, str]],
        max_duration: float = _MAX_SUB_DURATION,
        max_chars: int = _MAX_SUB_CHARS,
    ) -> List[List[Tuple[float, float, str]]]:
        """与 _group_items_to_srt 使用相同的贪心分组边界条件，但保留每组内部
        逐词的 (start_s, end_s, text) 列表，而非折叠为单个字符串 —— 用于
        karaoke 逐词高亮字幕渲染，需要保留组内每个词各自的起止时间。"""
        if not items:
            return []
        ...
        groups: List[List[Tuple[float, float, str]]] = [[items[0]]]
        ...
        while len(groups) >= 2:
            last_group = groups[-1]
            prev_group = groups[-2]
            last_dur = last_group[-1][1] - last_group[0][0]
            last_chars = sum(len(w[2]) for w in last_group)
    ...
    @staticmethod
    def _group_items_for_karaoke_no_tail_merge(
        items: List[Tuple[float, float, str]],
        max_duration: float = _MAX_SUB_DURATION,
        max_chars: int = _MAX_SUB_CHARS,
    ) -> List[List[Tuple[float, float, str]]]:
        """Same greedy grouping as _group_items_for_karaoke, WITHOUT the
        tail-merge step. That merge step exists to avoid orphan 1-word SRT
        lines in the ~4-7 word design this was originally tuned for, but for
        1-4 word karaoke captions it actively defeats the point: it
        re-combines short trailing groups (which, at this chunk size, is
        basically every group) back into long multi-word captions -- e.g. a
        full "I've had it a while." appearing as one caption instead of
        "I've had" / "it a" / "while." This is the grouping karaoke
        rendering should actually use; kept as a separate method rather than
        editing _group_items_for_karaoke in place so the plain-SRT path
        (_group_items_to_srt below) is unaffected.
        """
        if not items:
            return []
        ...
        for i in range(1, len(items)):
            ...
            prospective_dur = e_s - group_start_s
            prospective_chars = group_chars + len(txt)
            # Hard word-count ceiling in addition to duration/char bounds --
            # this is the actual "1-4 words max" requirement, enforced
            # directly rather than only approximated via char count.
            should_break = (
                prospective_dur > max_duration
                or prospective_chars > max_chars
                or group_word_count >= 4
                or (gap > 0.4 and group_chars > 4 and len(items) > 8)
            )
            if should_break:
                groups.append([(s_s, e_s, txt)])
        ...
        # Deliberately NO tail-merge step here.
        return groups

    @staticmethod
    def generate_karaoke_word_data(word_cues: list) -> list:
        """从 edge-tts 词级 cues 构建 karaoke 逐词高亮字幕所需的结构化数据。

        与生成 SRT 不同，这里保留每个词自己的起止时间，供渲染层在同一行
        文本中，随着朗读逐词切换高亮颜色。
        ...
        groups = SubtitleGenerator._group_items_for_karaoke_no_tail_merge(items)
        lines = []
        for group in groups:
            if not group:
                continue
    ...
    def generate_cue_aware_srt(
        ...
        """基于 TTS 词级 cues 的精确时间线对齐字幕生成（替代 _generate_scene_aware_srt）。
        ...
        Args:
            word_cues: TTS sub_maker（含 .cues）。
    ...
    def cues_to_srt(cues, output_path: str) -> str:
        """将 TTS 词级 cues 转换为 SRT 文件。

        优先使用词级 cues（Kokoro 原生 token 时间戳或本地强制对齐结果）进行细粒度分割，
        确保每 2-3 秒至少有一条字幕，避免出现 5 秒视频只有 1 条字幕的问题。

        若 sub_maker 提供 get_srt()/generate_subs()，回退到该接口解析。

        Args:
            cues: TTS sub_maker 实例或空 dict
        ...
        # sub_maker.cues 包含逐词时间戳（Kokoro 原生或强制对齐产出）
        ...
        # ── 策略 2: 回退到 sub_maker 自带的 SRT/VTT 生成接口 ──
        if not srt_content.strip():
            try:
                ...
                # 某些 sub_maker/srt 库版本的 Subtitle 对象结构不兼容
