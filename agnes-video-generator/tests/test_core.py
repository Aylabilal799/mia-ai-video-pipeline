...
class TestBuildPosition:
    ...
    NOTE: bare "top"/"bottom" used to collapse to the literal frame-edge
    tuple with ZERO margin, and any other string (including offset syntax
    like "bottom-300") was discarded entirely and also collapsed to
    "bottom". That was the root cause of captions rendering flush against
    the frame edge regardless of what was requested. Fixed: bare "top"/
    "bottom" now resolve to a safe default margin (not the edge), and any
    other recognized expression is passed through verbatim so offset
    syntax actually reaches resolve_position().
    """

    def test_top(self):
        from server import _build_position
        assert _build_position("top") == ("center", "top+80")

    def test_bottom(self):
        from server import _build_position
        assert _build_position("bottom") == ("center", "bottom-80")

    def test_default_is_bottom(self):
        from server import _build_position
        assert _build_position("") == ("center", "bottom-80")

    def test_offset_syntax_passed_through_verbatim(self):
        from server import _build_position
        assert _build_position("bottom-300") == ("center", "bottom-300")
        assert _build_position("top+50") == ("center", "top+50")
