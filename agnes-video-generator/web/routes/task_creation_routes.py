...
def _build_subtitle_config(
    ...
    subtitle_highlight_color: str = "#39FF14",
    subtitle_max_width_ratio: Optional[float] = None,
) -> SubtitleConfig:
    """构建独立字幕配置（v3.0）。"""
    style_kwargs = dict(
        font=subtitle_font,
    ...
    if subtitle_max_width_ratio:
        style_kwargs["max_width_ratio"] = subtitle_max_width_ratio
    subtitle_style = SubtitleStyle(**style_kwargs)
...
async def create_creative_task(
    ...
    # v4.0: 音色与目标语言兼容性校验
    if audio_enabled:
        helpers._validate_voice_compat(audio_voice, audio_lang or "en")
...
async def create_manuscript_task(
    ...
    # 固定角色身份锚定（可选）：本地图片路径 + 外观描述，用于跨分段/跨视频
    # 保持同一角色（如常驻 AI 网红）身份一致。
    reference_image: str = Form(""),
    character_style: str = Form(""),
    ...
    subtitle_stroke_width: int = Form(2),
    subtitle_bg_color: str = Form("black@0.5"),
    subtitle_highlight_color: str = Form("#39FF14"),
    subtitle_max_width_ratio: Optional[float] = Form(None),
):
    """创建稿件长视频任务（类型 3）。"""
    api_key = get_api_key()
    if not api_key:
    ...
    if audio_enabled:
        helpers._validate_voice_compat(audio_voice, audio_lang or "en", text=manuscript_text)
    ...
    subtitle_config = _build_subtitle_config(
        ...
        subtitle_font, subtitle_color, subtitle_fontsize, subtitle_position,
        subtitle_stroke_color, subtitle_stroke_width, subtitle_bg_color,
        subtitle_highlight_color, subtitle_max_width_ratio,
    )
...
async def create_anchor_task(
    ...
    if audio_enabled:
        helpers._validate_voice_compat(audio_voice, audio_lang or "en", text=script_text)
