...
def _build_position(subtitle_position: str) -> tuple:
    """将字幕位置字符串转为 moviepy 兼容的 (h, v) 位置元组。

    FIX: this previously collapsed EVERY non-"top" input (including offset
    syntax like "bottom-300") down to the bare literal ("center", "bottom").
    SubtitleGenerator.resolve_position() only special-cases bare "top"/
    "bottom" as flush against the frame edge (0px margin) -- it's the
    "bottom-N"/"top+N" offset syntax that actually produces a safe margin.
    Since this function discarded any offset before it ever reached
    resolve_position(), captions always rendered flush against the very
    bottom edge of the frame regardless of what was requested, which is
    exactly the "captions too close to the bottom" bug.

    Now:
      - bare "top"/"bottom"/"" fall back to a safe default margin (not the
        frame edge) via DEFAULT_SUBTITLE_TOP_MARGIN_PX / DEFAULT_SUBTITLE_
        BOTTOM_MARGIN_PX in core/config.py, so nothing needs a bug fix
        elsewhere just to render safely.
      - anything else (e.g. "bottom-300", "top+50", "center", "top-left")
        is passed through verbatim -- resolve_position() already knows how
        to parse all of these.
    """
    from core.config import DEFAULT_SUBTITLE_BOTTOM_MARGIN_PX, DEFAULT_SUBTITLE_TOP_MARGIN_PX

    sp = (subtitle_position or "").strip().lower()
    if sp == "top":
        return ("center", f"top+{DEFAULT_SUBTITLE_TOP_MARGIN_PX}")
    if sp in ("", "bottom"):
        return ("center", f"bottom-{DEFAULT_SUBTITLE_BOTTOM_MARGIN_PX}")
    return ("center", subtitle_position.strip())
...
async def _get_or_generate_preview(voice_id: str, text: str) -> str:
    """获取试听音频：缓存命中直接返回路径，否则调用 Kokoro 生成后缓存。
    ...
    from core.audio.tts import KokoroTTSEngine

    tmp_path = cache_path + ".tmp"
    await KokoroTTSEngine().generate(text=text, output_path=tmp_path, voice=voice_id)
...
def _resolve_preview_text(voice_id: str, text: str) -> str:
    ...
    if text:
        return text
    vlang = get_voice_lang(voice_id) or "en"
    # Kokoro voice ids look like "af_heart" -> display name "heart";
    # legacy edge_tts-style ids ("...Neural") are handled for back-compat.
    name = voice_id.split("_")[-1].replace("Neural", "") if "_" in voice_id else voice_id
    return VOICE_PREVIEW_TEXTS.get(vlang, VOICE_PREVIEW_TEXTS["en"]).format(name=name)
